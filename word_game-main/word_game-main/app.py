...
def generate_word_hints_text(level_info, missing_words, incorrect_words, user_sentence, correct_words, sentence_prompt) -> str:
    ...
    critique_prompt = (
        ...
        "2b. 請特別檢查標點與空格的使用是否正確（例如不要在標點前加空格、標點後應有一個空格、避免連續空格），若有錯誤請修正並簡短指出。\n"
        "3. 提醒學生還沒猜對所有單字，鼓勵他們嘗試使用已猜出的單字造句。\n"
        "回覆格式：請直接輸出文法建議和鼓勵，總長度限制在 50 到 140 個中文字之間。"
    )
...
def analyze_user_sentence_text(user_sentence: str, correct_words: list, sentence_prompt: str) -> str:
    ...
    # 建構給 Gemini 模型的詳細提示 (強制檢查單字和句型，並特別檢查單複數、主謂一致與標點/空格使用)
    prompt = (
        ...
        "d) **請特別檢查** 單字的單數/複數形式及主詞與動詞的一致性（主謂一致），若有錯誤請在回覆中明確指出。\n"
        "e) **請特別檢查** 標點與空格使用是否正確（例如不要在標點前加空格，標點後應有一個空格；避免連續空格）。若有錯誤請修正並短述示例。\n\n"
        "🎯 關鍵要求：若句子在單字使用、句型合規與文法（尤其單複數、主謂一致與標點/空格）上**完全正確**，請在回覆一開始以『恭喜你完全答對了！』開頭，並在括號中標註『(單複數與主謂一致與標點/空格皆正確)』，例如：『恭喜你完全答對了！(單複數與主謂一致與標點/空格皆正確)』。總長度限制在 50 到 140 個中文字之間。請直接輸出回饋內容，不包含額外標題。"
    )
